...
class OneDollarRecognizer(IvyServer):
...
if __name__ == "__main__":
    ...
    icon = pygame.image.load('OneDollarIvy\data\onedollar.jpg')
    ...
    while not done:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                ODI.positions = [(x, y, 'start')]
                ODI.OnPaint(event.button)
                pos = pygame.mouse.get_pos()
                
            elif event.type == pygame.MOUSEMOTION:
                if ODI.mouseDown:
                    (x, y) = pygame.mouse.get_pos()
                    ODI.positions.append((x, y, 'move'))
                    ODI.OnPaint(None)
                
            elif event.type == pygame.MOUSEBUTTONUP:
                ODI.mouseDown = False
                (x, y) = pygame.mouse.get_pos()
                ODI.positions.append((x, y, 'stop'))
                ODI.OnPaint(event.button)
    ...
    if len(ODI.extra_trainingPoints)>0:
        print("=======Copy below print to put as input=======",end="\n")
        for i, one_template in enumerate(ODI.extra_trainingPoints):
